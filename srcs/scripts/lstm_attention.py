...
def RecComModel():
    ...
    x = MaxPooling1D(4)(x)
    x = Flatten()(x)
    x = Dense(8, activation='softmax', name='fc0', kernel_regularizer=regularizers.L2(1e-2))(x)
    model = Model(inputs = x_input, outputs = x)
    model.summary()
    return model


output_path = path + 'Results_attention_2'
model = RecComModel()
...
model.compile(optimizer=optimizers.Adam(learning_rate=1e-6), loss='categorical_crossentropy', metrics=['accuracy'])
