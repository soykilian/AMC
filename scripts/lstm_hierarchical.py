...
def RecComModel():
    ...
    x = Dense(13, activation='softmax', name='fc0')(x)
    model = Model(inputs = x_input, outputs = x)
    model.summary()
    return model


output_path = path + 'Results_hierarchical'
model = RecComModel()
clr_triangular = CyclicLR(mode='triangular', base_lr=1e-7, max_lr=1e-3, step_size= 4 * (X_train.shape[0] // 256))
...
Train = True
if Train:
    history = model.fit(X_train, Y_train, epochs = 200, batch_size = 256, callbacks = c, validation_data=(X_val, Y_val))
